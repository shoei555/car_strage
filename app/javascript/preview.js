document.addEventListener('turbo:load', function(){
  ...
  const buildPreviewImage = (dataIndex, blob) =>{
    ...
    previewImage.setAttribute('width', '150'); // 幅を指定
    previewImage.setAttribute('height', '200');// 高さを指定
    ...
    deleteButton.style.width = "60px"; // 幅を指定
    deleteButton.style.height = "40px"; // 高さを指定
    deleteButton.style.color = "white";
    deleteButton.style.backgroundColor = "black";
    deleteButton.style.borderRadius = "30%"; // 丸くするための角丸の半径
  ...
  const buildNewFileField = () => {
    ...
    newFileField.setAttribute('name', 'car[images][]');
    newFileField.style.width = "280px"; 
    newFileField.style.height = "30px";
  

    // 最後のfile_fieldを取得
    const lastFileField = document.querySelector('input[type="file"][name="car[images][]"]:last-child');
